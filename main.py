...
# ╔══════════════════════════════════════════════════╗
# ║     MAHIR — Auto Dependency Installer     ║
# ║     app.py রান করলেই সব install হয়ে যাবে!      ║
# ╚══════════════════════════════════════════════════╝

def _MAHIR_auto_install():
    PACKAGES = [
        "Flask==3.0.0", "gunicorn", "requests", "httpx",
        "aiohttp", "urllib3", "psutil", "pytz", "PyJWT",
        "pycryptodome", "colorama", "rich", "loguru",
        "flask_cors", "cachetools", "pyTelegramBotAPI",
        "python-telegram-bot==20.7", "aiogram",
        "google", "google_play_scraper", "pyngrok",
        "protobuf-decoder",
    ]
    for pkg in PACKAGES:
        base = pkg.split("=")[0].split(">")[0].split("<")[0].replace("-","_").lower()
        try:
            __import__(base)
        except ImportError:
            subprocess.run(
    ...
    # py7zr — Termux-এ Rust লাগে তাই try করি, না হলে skip
    try:
        import py7zr
    except ImportError:
        for ver in ["py7zr==0.20.8", "py7zr==0.21.0", "py7zr"]:
            r = subprocess.run(
                [sys.executable, "-m", "pip", "install", ver,
                 "--quiet", "--no-warn-script-location"],
                capture_output=True
            )
            if r.returncode == 0:
...
import shutil
import zipfile
# ── py7zr safe import (Termux compatible) ──
try:
    import py7zr
    _PY7ZR_OK = True
except ImportError:
...
DEFAULT_CONFIG = {
    "site_title": "MAHIR PREMIUM VPS",
    "site_header": "MAHIR VPS",
    ...
    try:
        ...
        if file and file.filename:
            ...
            if file.filename.lower().endswith('.zip'):
                ...
                try:
                    if not _PY7ZR_OK:
                        print("7z not supported: py7zr not installed")
                    else:
...
def extract_archive(server_id, filename):
    try:
        ...
        if filename.lower().endswith('.zip'):
            ...
            if not _PY7ZR_OK:
                raise Exception("7z not supported: py7zr not installed")
...
def upload_file(server_id):
    try:
        ...
        if file.filename.lower().endswith('.zip'):
            ...
            try:
                if not _PY7ZR_OK:
                    return jsonify({'status': 'ok', 'warning': '7z not supported on this system'})
...
def send_api_response(message):
    ...
    try:
        r = requests.get(url, timeout=15, headers={{'User-Agent': 'MAHIR-TeleBot'}})
...
def update_config():
    try:
        ...
        HARDCODED_ADMIN_PASS = "FFBY7X"
        
        admin_pass = request.form.get('admin_password')
        if not admin_pass:
            return jsonify({'error': 'Admin password required'}), 403
        
        # সরাসরি স্ট্রিং তুলনা করুন (হ্যাশিং ছাড়া)
        if admin_pass != HARDCODED_ADMIN_PASS:
...
#    _t = ngrok.connect(addr=8080, proto="http")
    ...
    print("=" * 50)
    print("MAHIR PREMIUM VPS - Starting...")
    print("=" * 50)
    print(f"Port: {port}")
